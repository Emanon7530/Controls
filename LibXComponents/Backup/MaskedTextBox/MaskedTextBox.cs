...
//				m_mask = Masked;
		...
		private void OnKeyPress(object sender, KeyPressEventArgs e)
		{
			...
			switch(m_mask)
			{
				...
				case Mask.IpAddress:
					...
					break;
				case Mask.SSN:
					//sd.MaskPhoneSSN(e, 3, 13);
					sd.MaskPhoneSSN(e,3,5);
					break;
				case Mask.Decimal:
					sd.MaskDecimal(e);
					break;
			...
			switch(m_mask)
			{
				case Mask.DateOnly:
					...
					if(!regStr.IsMatch(sd.Text))
						errorProvider1.SetError(this, "Formato de fecha invalido; dd/mm/yyyy");
					break;
				case Mask.PhoneWithArea:
					regStr = new Regex(@"\d{3}-\d{3}-\d{4}");
					if(!regStr.IsMatch(sd.Text))
						errorProvider1.SetError(this, "Telefono invalido; xxx-xxx-xxxx");
					break;
				case Mask.IpAddress:
					short cnt=0;
					...
					for(short i=0; i<len;i++)
						if(sd.Text[i] == '.')
						{
							...
							if(i+1 < len)
								if(sd.Text[i+1] == '.')
								{
									errorProvider1.SetError(this, "Direccion IP invalida; x??.x??.x??.x??");
									break;
								}
						}
					...
					break;
				case Mask.SSN:
					regStr = new Regex(@"\d{3}-\d{7}-\d{1}");
					if(!regStr.IsMatch(sd.Text))
						errorProvider1.SetError(this, "SSN is not valid; xxx-xxxxxxx-x");
					break;
				case Mask.Decimal:
					break;
				case Mask.Digit:
		...
		private void MaskDigit(KeyPressEventArgs e)
		{
			...
			if (e.KeyChar == (char)3 || e.KeyChar == (char)22 || e.KeyChar == (char)24 || e.KeyChar ==(char)26  || e.KeyChar ==(char)13)
			{
				e.Handled = false;
				return;
			}
		...
		private void MaskSNN(KeyPressEventArgs e)
		{
			int len = this.Text.Length;
			int indx = this.Text.LastIndexOf("-");
			...
			{ // only digit, Backspace and dot are accepted
				...
				if (e.KeyChar != 8)
				{
					...
					{   // added - MAC
						// process the "."
						if (DelimitNumber + 1 > 2) // cant have more than 3 dots (at least for IPv4)
						{
