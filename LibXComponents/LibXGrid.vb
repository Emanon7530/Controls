...
Public Class LibXGrid
    Inherits DataGrid
    Private _RedOnlyState() As Boolean
    ...
    Friend mFirstAfterAutoSearch As Boolean
    Dim mForm As Form
    Private mAutoAdjustLastColumn As Boolean = True
    Private mUseHandCursor As Boolean
    Private m_objTableFot As DataTable
    Private mWasTab As Boolean
    ...
    Sub New()
        ...
        If Not Me.DesignMode Then
            ...
            If Not Me.DesignMode And Me.mUseAuotFillLines Then
    ...
    Public Sub SetValue(ByVal row As Integer, ByVal col As Integer, ByVal value As Object)
        Try
            ...
            '-->Creo que no hay necesidad de ponerlo en este nivel
            Dim oCol As Object
            oCol = Me.TableStyles(0).GridColumnStyles(col)
            If TypeOf oCol Is XDataGridComboBoxColumn Then
                CType(oCol, XDataGridComboBoxColumn).ComboBox.SelectedValue = (Trim(value))
            End If
    ...
    Public Function GetValue(ByVal col As DataGridColumnStyle) As String
        Try
            ...
            If TypeOf col Is XDataGridComboBoxColumn Then
                oValue = CType(col, XDataGridComboBoxColumn).ComboBox.SelectedValue
            Else
                oValue = Me(Me.CurrentRowIndex, Me.GetColNum(col))
            End If
    ...
    Public Function GetValue(ByVal row As Integer, ByVal col As DataGridColumnStyle) As String
        Try
            ...
            If TypeOf col Is XDataGridComboBoxColumn Then
                oValue = CType(col, XDataGridComboBoxColumn).ComboBox.SelectedValue
            Else
                oValue = Me(row, Me.GetColNum(col))
            End If
    ...
    Protected Overrides Sub OnCurrentCellChanged(ByVal e As System.EventArgs)
        ...
        Try
            ...
            If m_blnOkToValidate And Not strValue Is Nothing Then
                If Not m_objDataSource Is Nothing AndAlso m_objDataSource.IsEditing = True Then

                    If Trim(strValue) = "" And Not m_blnStartEdit Then
                        m_blnStartEdit = True
                    End If
            ...
            If mWasTab Then
                If Not m_objDataSource Is Nothing AndAlso m_objDataSource.IsEditing = True Then
                    If Me.TableStyles.Count > 0 AndAlso Me.TableStyles(0).GridColumnStyles.Count > 0 Then
                        If Me.TableStyles(0).GridColumnStyles(Me.m_intNewCurrentCol).Width = 0 _
                        Or Me.TableStyles(0).GridColumnStyles(Me.m_intNewCurrentCol).ReadOnly Then
                            SendKeys.Send("{TAB}")
                            mWasTab = False
                        Else
                            If TypeOf Me.TableStyles(0).GridColumnStyles(Me.m_intNewCurrentCol) Is XEditTextBoxColumn _
                            Or TypeOf Me.TableStyles(0).GridColumnStyles(Me.m_intNewCurrentCol) Is XDataGridTextButtonColumn _
                            Or TypeOf Me.TableStyles(0).GridColumnStyles(Me.m_intNewCurrentCol) Is XDataGridLinkColumn Then
                                Dim oCol As Object
                                oCol = Me.TableStyles(0).GridColumnStyles(Me.m_intNewCurrentCol)
                                If oCol.isReadOnly Or oCol.tabstop = False Then
                                    SendKeys.Send("{TAB}")
                                    mWasTab = False
                                End If
                            End If
    ...
    Protected Overloads Overrides Sub ColumnStartedEditing(ByVal editingControl As System.Windows.Forms.Control)
        Try
            If TypeOf Me.m_objDataSource Is LibXConnector Then
                If Me.m_objDataSource.IsEditing = False Then
                    Exit Sub
                End If
            Else
                Exit Sub
    ...
    Protected Overrides Sub OnDataSourceChanged(ByVal e As System.EventArgs)
        ...
        Try
            ...
            If m_objTable Is Nothing Then
                ...
                If TypeOf m_objDataSource Is LibXConnector Then

                    RemoveHandler m_objDataSource.AfterRowChange, AddressOf onAfterDsRowChange
                    RemoveHandler m_objDataSource.ChangeState, AddressOf onDsChangeState

                    AddHandler m_objDataSource.AfterRowChange, AddressOf onAfterDsRowChange
                    AddHandler m_objDataSource.ChangeState, AddressOf onDsChangeState
                    '-->AddHandler objNS.AfterLoadDetail, AddressOf onDsAfterLoadDetail
                ...
                If m_blnShowFooter And Not m_objTable Is Nothing Then
                    ...
                    For Each objCol In m_objTable.Columns
                        ...
                        oCols.ColType = objCol.DataType 'GetType(String) 
                    ...
                    If Me.TableStyles.Count > 0 Then
                        ...
                        For Each objGst In Me.TableStyles(0).GridColumnStyles
                            ...
                            objGstL = New XEditTextBoxColumn
                            objGstL.MappingName = objGst.MappingName
                            If TypeOf objGst Is DataGridTextBoxColumn _
                            Or TypeOf objGst Is XEditTextBoxColumn Then
                    ...
                    Else
                        ...
                        For Each objCol In m_objTable.Columns
                            ...
                            If objCol.ColumnMapping <> MappingType.Hidden Then
                                ...
                                If objCol.DataType Is GetType(Decimal) Then
                                    objGstL.Format = "###,###,##0.00"
                                End If
                    ...
                    m_objFooterGrid.ReadOnly = True
                    m_objFooterGrid.IsReadOnly = True
            ...
            Me.ReadOnly = True
            'ChangeReadOnlyState(LibxConnectionActions.None)
    ...
    'Private Sub ChangeReadOnlyState(ByVal action As LibX.LibxConnectionActions)
    '    ReDim Preserve _RedOnlyState(Me.TableStyles(0).GridColumnStyles.Count)

    '    Select Case action
    '        Case LibxConnectionActions.Add, LibxConnectionActions.Edit, LibxConnectionActions.Find
    '            For Each oControl As DataGridColumnStyle In Me.TableStyles(0).GridColumnStyles
    '                oControl.ReadOnly = _RedOnlyState(Me.TableStyles(0).GridColumnStyles.IndexOf(oControl))
    '            Next

    '        Case Else
    '            For Each oControl As DataGridColumnStyle In Me.TableStyles(0).GridColumnStyles
    '                _RedOnlyState(Me.TableStyles(0).GridColumnStyles.IndexOf(oControl)) = ocontrol.readonly
    ...
    Private Sub onDsChangeState(ByVal sender As Object, ByVal e As XChangeStateEventArgs)
        ...
        If mBack.Equals(Color.Empty) Then
            If Me.TableStyles.Count > 0 Then
                mBack = Me.TableStyles(0).BackColor
            Else
                mBack = Me.BackColor
            End If
        End If

        '        ChangeReadOnlyState(e.action)

        Select Case e.action
            Case LibxConnectionActions.Add
                If MdlUtil.UseEditingColors Then
                    If Me.TableStyles.Count > 0 Then
                        Me.BackColor = Color.Red
                        Me.TableStyles(0).BackColor = SystemColors.Info
                        Me.TableStyles(0).AlternatingBackColor = SystemColors.Window
                    Else
            ...
            Case Else
                If MdlUtil.UseEditingColors Then
                    If Me.TableStyles.Count > 0 Then
                        Me.TableStyles(0).BackColor = mBack
                    Else
                        Me.BackColor = mBack
                    End If
                End If
    ...
    Private Sub onDsAfterLoadDetail(ByVal sender As Object, ByVal e As XbeforeLoadDetailEventArgs)
        If Me.m_blnShowFooter And Not Me.m_objFooterGrid Is Nothing Then
            Me.refreshFooter()
        End If
    End Sub
    ...
    Private Sub onFooterChanging(Optional ByVal clearGrid As Boolean = False)
        ...
        Try
            ...
            '// Crear la tabla Footer y poner 
            '// todos los campos permitir Nulos
            Dim oC As DataColumn
            If m_objTableFot Is Nothing Then
                m_objTableFot = m_objTable.Clone
                For Each oC In m_objTableFot.Columns
                    oC.AllowDBNull = True
                Next
            End If

            '// Limpiar la tabla Footer
            m_objTableFot.Rows.Clear()

            '''Dim oRow As DataRowView
            '''Dim oRow2 As DataRow

            ''''// Agregar todas las filas del grid principal
            ''''// al grid footer
            '''For Each oRow In Me.getCurrentGridView
            '''    oRow2 = m_objTableFot.NewRow

            '''    For Each oC In m_objTable.Columns
            '''        oRow2(oC.ColumnName) = oRow(oC.ColumnName)
            '''    Next
            '''    m_objTableFot.Rows.Add(oRow2)
            '''Next
            ...
            ''''    objResult.ColumnResult = m_objTableFot.Compute(objCol.Expression, objCol.Filter)
    ...
    Public Function GetFooterValue(ByVal col As DataGridColumnStyle) As Object
        Dim oValue As Object

        If Not m_objFooterTable Is Nothing Then
            oValue = m_objFooterTable.Rows(0).Item(col.MappingName)
        End If
                ...
                Else
                    ...
                    Case 32
                        ...
                        Case 27
                            ...
                            If Me.getCurrentGridView.Count > 0 Then
                                Me.Focus()
                                Me.CurrentCell = New DataGridCell(0, 0)
                            End If
                    End Select
                End If
                'Else
                '    If Char.IsLetterOrDigit(kpe.KeyChar) Then
                '        If IsValidChar(kpe.KeyChar) Then
                '            Me.searchText += kpe.KeyChar
                '            setFilter(Me.searchText.trim & "%")
                '        End If
                '    End If
            End If
    ...
    Public Property AutoAdjustLastColumn() As Boolean
        Get
            Return mAutoAdjustLastColumn
        End Get
        Set(ByVal Value As Boolean)
            mAutoAdjustLastColumn = Value
        End Set
    End Property
    ...
    Public Sub AdjustColumnSizeEx(ByVal p_blnInForce As Boolean, ByVal checkStringWidh As Boolean)
        Try
            ...
            If Not mAutoAdjustLastColumn Then
                Exit Sub
            End If
                    ...
                    If Me.TableStyles(0).GridColumnStyles(i).Width > 0 Then
                        If dataTable1.Columns.Contains(Me.TableStyles(0).GridColumnStyles(i).MappingName) Then
                            ...
                            If checkStringWidh Then
                                ...
                                If i > 0 Then
                                    If dataTable1.Columns(Me.TableStyles(0).GridColumnStyles(i).MappingName).DataType.Equals(GetType(System.String)) _
                                    Or dataTable1.Columns(Me.TableStyles(0).GridColumnStyles(i).MappingName).DataType.Equals(GetType(System.Char)) Then
...
#End Region
    ...
    Protected Overrides Function ProcessCmdKey(ByRef msg As System.Windows.Forms.Message, ByVal keyData As System.Windows.Forms.Keys) As Boolean
        Try
            ...
            '// EMR (Modificado el 09/07/2008)
            If msg.WParam.ToInt32 = Keys.Enter _
            Or (Me.ReadOnly = False AndAlso msg.WParam.ToInt32 = Keys.Down) Then
                Me.ResetautoSearch()
                If Not mFirstAfterAutoSearch Then
                    mWasTab = True
                    SendKeys.Send("{Tab}")
                    Return True
                End If
                mFirstAfterAutoSearch = False
            End If

            '-->Return MyBase.ProcessCmdKey(msg, keyData)
            Dim b As Boolean
            b = MyBase.ProcessCmdKey(msg, keyData)
            mWasTab = False
        ...
        Finally
            ...
            If Not Me.isAutoSearching Then
                ...
                '''Try
                '''    If m_objDataSource.IsDataEditing = True Then
                '''        If keyData = Keys.Escape Then
                '''            Me.Focus(Me.CurrentRowIndex, 0)
                '''        End If
                '''    End If
                '''Finally
                '''End Try
    ...
    Protected Overrides Sub OnPaint(ByVal pe As System.Windows.Forms.PaintEventArgs)
        ...
        Do While j <= Me.VisibleColumnCount - 1
            If fr > 0 Then
        ...
        Dim maxLines As Integer = (oRect.Height / Me.PreferredRowHeight) - fr
    ...
    Private Sub LibXGrid_CurrentCellChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.CurrentCellChanged

        'If MyClass.TableStyles(0).GridColumnStyles(MyClass.CurrentCell.ColumnNumber).ReadOnly = True Then
        '    SendKeys.Send("{Tab}")
        'End If
    End Sub
    ...
    Protected Overrides Sub WndProc(ByRef m As System.Windows.Forms.Message)
        Try
            ...
            If Me.mUseHandCursor Then
                Select Case m.Msg
                    Case WM_MOUSEMOVE

                        'Dim r As Rectangle = Me.ClientRectangle
                        'Dim pt As Point = PointToScreen(New Point(r.X, r.Y))
                        'r.X = pt.X
                        'r.Y = pt.Y
                        'If r.Contains(control.MousePosition ) Then
                        Dim r As Rectangle = Me.ClientRectangle
    ...
    Public Overloads Function Focus(ByVal RowIndex As Integer, ByVal ColumnIndex As Integer) As Boolean
        Me.CurrentCell = New DataGridCell(RowIndex, ColumnIndex)
        Me.Focus()
        End Function
